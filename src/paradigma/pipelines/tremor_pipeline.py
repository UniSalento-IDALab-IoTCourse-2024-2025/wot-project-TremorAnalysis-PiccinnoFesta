...
def extract_tremor_features(df: pd.DataFrame, config: TremorConfig) -> pd.DataFrame:
    ...
    windowed_data = tabulate_windows_safe(df, windowed_cols, config.window_length_s, config.window_step_length_s, config.sampling_frequency)
    ...
    print("Num windows passed to spectral features:", windowed_gyro.shape[0])
...
def extract_tremor_features_safe(df: pd.DataFrame, config: TremorConfig) -> pd.DataFrame:
    """
    A safe and improved version of extract_tremor_features using tabulate_windows_safe,
    which ensures all data windows are processed and accurate timestamps are retained.

    Parameters
    ----------
    df : pd.DataFrame
        Preprocessed input data containing timestamps and gyroscope columns.

    config : TremorConfig
        Configuration with gyroscope column names, windowing parameters, and sampling frequency.

    Returns
    -------
    pd.DataFrame
        DataFrame of extracted tremor features, each row aligned to the start time of its window.
    """
    # Columns to use: time + gyroscope
    windowed_cols = [DataColumns.TIME] + config.gyroscope_cols

    # Get windowed data and start times
    windowed_data, start_times = tabulate_windows_safe(
        df,
        windowed_cols,
    ...
    print("start_times[0]:", start_times[0])
    print("start_times[-1]:", start_times[-1])
    print("start_times dtype:", type(start_times[0]))

    # Extract gyroscope-only data (exclude time column)
    windowed_gyro = windowed_data[:, :, 1:]  # assumes time is first, then gyro_x/y/z

    # Prepare output DataFrame
    df_features = pd.DataFrame(start_times, columns=[DataColumns.TIME])
...
def detect_tremor(df: pd.DataFrame, config: TremorConfig, full_path_to_classifier_package: str | Path) -> pd.DataFrame:
    ...
    #aggiunta mia per cammbiare la soglia, di prova
    if hasattr(config, 'logreg_threshold'):
        clf_package.threshold = config.logreg_threshold
