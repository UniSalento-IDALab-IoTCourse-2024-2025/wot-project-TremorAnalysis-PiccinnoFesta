...
def tabulate_windows_safe(
    ...
    window_size = int(window_length_s * fs)
    step_size = int(window_step_length_s * fs)
    n_columns = len(columns)

    data = df[columns].values
    timestamps = df[DataColumns.TIME].tolist()  # ✅ mantiene datetime + timezone
    ...
    for start in range(0, total_samples - window_size + 1, step_size):
        ...
        start_times.append(timestamps[start])  # ✅ prende direttamente timestamp dal DataFrame

    if not windows:
        return np.empty((0, window_size, n_columns)), np.empty((0,))
